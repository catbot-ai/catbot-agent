...
pub fn draw_chart(
    ...
    let candle_width;
    let (top, bottom) = if chart.stoch_rsi_enabled {
        let (stoch_rsi_area, main_and_bottom) = root.split_vertically((25).percent());
        let (main_area, bottom_area) = main_and_bottom.split_vertically((67).percent());

        let mut temp_chart =
            ChartBuilder::on(&main_area).build_cartesian_2d(first_time..last_time, 0f32..1f32)?;
        candle_width = calculate_candle_width(&temp_chart, all_candle_data.len() as u8);

        let (first_visible_time, last_visible_time, visible_data) =
            get_visible_range_and_data(all_candle_data, timezone, candle_width, final_width * 2)?;
        ...
        stoch_rsi_chart.draw_series(LineSeries::new(
            ...
            ShapeStyle::from(&SRSI_K).stroke_width(1),
        ))?;
        stoch_rsi_chart.draw_series(LineSeries::new(
            stoch_rsi_lines.iter().map(|(t, _, d)| (*t, *d)),
            ShapeStyle::from(&SRSI_D).stroke_width(1),
        ))?;
        ...
        stoch_rsi_chart.draw_series(LineSeries::new(
            vec![(first_visible_time, 80.0), (last_visible_time, 80.0)],
            dash_style,
        ))?;
        stoch_rsi_chart.draw_series(LineSeries::new(
            vec![(first_visible_time, 20.0), (last_visible_time, 20.0)],
            dash_style,
        ))?;

        (main_area, bottom_area)
    } else {
        let (main_area, bottom_area) = root.split_vertically((50).percent());
        let mut temp_chart =
            ChartBuilder::on(&main_area).build_cartesian_2d(first_time..last_time, 0f32..1f32)?;
        candle_width = calculate_candle_width(&temp_chart, all_candle_data.len() as u8);
        (main_area, bottom_area)
    ...
    if chart.volume_enabled || chart.macd_enabled {
        let num_indicators = [chart.volume_enabled, chart.macd_enabled]
            .iter()
...
pub fn draw_axis_labels(
    ...
    let mut maybe_bounding_rect: Option<Rect> = None;

    // Define section heights
    let stoch_rsi_section_height = if chart.stoch_rsi_enabled {
        height as f32 * 0.25
    } else {
        0.0
    };
    let main_chart_section_height = if chart.stoch_rsi_enabled {
        height as f32 * 0.42
    } else {
        height as f32 * 0.5
    };

    let remaining_height = height as f32 - stoch_rsi_section_height - main_chart_section_height;
    let num_bottom_indicators = [chart.volume_enabled, chart.macd_enabled]
        .iter()
        .filter(|&&e| e)
        .count() as f32;
    let bottom_section_height = if num_bottom_indicators > 0.0 {
        remaining_height / num_bottom_indicators
    } else {
        0.0
    };
    ...
    if chart.stoch_rsi_enabled {
        let stoch_rsi_step = 100.0 / 2.0; // For 100, 50, 0
        let y_positions = [
            current_y,
            current_y + stoch_rsi_section_height * 0.5,
            current_y + stoch_rsi_section_height - text_height as f32,
        ];
        for (i, y) in y_positions.iter().enumerate() {
            let value = 100.0 - (i as f32 * stoch_rsi_step);
            draw_label(
                img,
                font,
                &format!("{:.0}", value),
        ...
        current_y += stoch_rsi_section_height;
    }

    // 2. Main Chart Price Labels
    let top_section_height = main_chart_section_height;
    if let Some(last_candle) = klines.last() {
        ...
        if price_range_adjusted > 0.0 {
            let normalized_position = (current_price - adjusted_min_price) / price_range_adjusted;
            let y_position = (current_y + (top_section_height * (1.0 - normalized_position)))
                as i32
                - text_height / 2;
            let y_position_clamped = y_position
                .max(current_y as i32)
                .min((current_y + top_section_height - text_height as f32) as i32);
            maybe_bounding_rect = Some(draw_label(
                img,
                font,
    ...
    current_y += main_chart_section_height;

    // 3. Volume Labels
    if chart.volume_enabled {
        let section_height = bottom_section_height;
