...
pub fn draw_chart(
    ...
    let (top_area, bottom) = if chart.stoch_rsi_enabled {
        root.split_vertically((70).percent())
    } else {
        root.split_vertically((50).percent())
    };

    let (stoch_rsi_area, main_chart_area) = if chart.stoch_rsi_enabled {
        let (stoch_area, main_area) = top_area.split_vertically((30).percent());
        (Some(stoch_area), main_area)
    } else {
        (None, top_area)
    };

    let mut top_chart = ChartBuilder::on(&main_chart_area)
    ...
    if let Some(stoch_rsi_area) = stoch_rsi_area {
...
pub fn draw_axis_labels(
    ...
    let (stoch_rsi_section_height, top_section_height, bottom_section_height) =
        if chart.stoch_rsi_enabled {
            (
                height as f32 * 0.7 * 0.3,
                height as f32 * 0.7 * 0.7,
                height as f32 * 0.3,
            )
        } else {
            (0.0, height as f32 * 0.5, height as f32 * 0.5)
        };
    ...
    let bottom_indicator_section_height = if num_bottom_indicators > 0.0 {
        bottom_section_height / num_bottom_indicators
    } else {
        0.0
    };
    ...
    if chart.stoch_rsi_enabled {
        ...
        let stoch_rsi_y_positions = [
            0.0,
            stoch_rsi_section_height * 0.5,
            stoch_rsi_section_height - text_height as f32,
        ];
    ...
    let maybe_bounding_rect = if let Some(last_candle) = klines.last() {
        ...
        let y_position = stoch_rsi_section_height as i32
            + 2
            + (top_section_height * (1.0 - normalized_position)) as i32
            - text_height / 2;

        // Constrain y-position to stay within the candlestick section
        let y_position_clamped = y_position
            .max(stoch_rsi_section_height as i32)
            .min((stoch_rsi_section_height + top_section_height - text_height as f32) as i32);
    ...
    let mut current_y = stoch_rsi_section_height + top_section_height;
    ...
    if chart.volume_enabled {
        ...
        let volume_y_positions = [
            current_y,
            current_y + bottom_indicator_section_height * 0.5,
            current_y + bottom_indicator_section_height - text_height as f32,
        ];
        ...
        current_y += bottom_indicator_section_height;
    }
