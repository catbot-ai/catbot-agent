    ...
    router
        .get_async("/suggest/:token", |_req, ctx| async move {
            if let Some(symbol) = ctx.param("token") {
                let output_result =
                    predict_with_gemini(gemini_api_key.to_owned(), symbol.to_owned()).await;
...
pub async fn predict_with_gemini(
    ...
    let prediction_result = get_prediction(&symbol, &provider, &gemini_model).await;
...
mod tests {
    ...
    async fn test() {
        ...
        let symbol = "SOLUSDT";
        let result = predict_with_gemini(gemini_api_key, symbol.to_string())
            .await
            .unwrap();
        println!("{:#?}", result);
        println!(
